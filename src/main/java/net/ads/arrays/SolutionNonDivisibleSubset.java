...
public class SolutionNonDivisibleSubset {
	...
	public static void main(String[] args) {
		...
		s = Arrays.asList(278, 576, 496, 727, 410, 124, 338, 149, 209, 702, 282, 718, 771, 575, 436);
	...
	public static int nonDivisibleSubset(int k, List<Integer> s) {
		...
		for (int i = 1; i <= k / 2; i++) {
			subsets += Math.max(remainders[i], remainders[k - i]);
		}
	...
	public static int nonDivisibleSubset2(int k, List<Integer> s) {
		...
		for (int i = 0; i < s.size(); i++) {
			for (int j = 0; j < s.size(); j++) {
				...
				System.out.println("i,j: " + s.get(i) + "," + s.get(j));
			}
		}
